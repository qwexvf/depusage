// Package extract holds the shared types used by depusage's public
// API and its per-language sub-packages. Splitting the types out here
// breaks the import cycle that would otherwise exist between the root
// package (which dispatches by Language) and the per-language
// extractors (which need to construct Result/Import values).
//
// External callers should use the re-exported aliases in package
// depusage; the path under internal/ is not part of the API.
package extract
